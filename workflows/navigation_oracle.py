"""
workflows/navigation_oracle.py — Oracle-guided navigation workflow for DroneAI.

Sequential oracle deliberation at each waypoint step.
The oracle monitors battery trend and telemetry freshness mid-navigation
and can inject WAIT or REPLAN before arrival confirmation.

Sequence:
  1. set_speed
  2. set_yaw (optional)
  3. goto_position  ← oracle most critical here
...
async def _deliberate(tool_name: str, resp, sm, safety) -> dict:
    """Build context and run oracle deliberation. Returns ThinkingDecision."""
...
async def run_navigation_oracle(
    ...
    """
    Oracle-guided point-to-point navigation.
    Drop-in replacement for workflows/navigation.py run_navigation().
    """
    logger.info(
        f"[NAV:ORACLE] Navigating to ({lat:.6f}, {lon:.6f}, {alt}m) "
        f"@ {speed_ms} m/s"
    )
    ...
    # ── Step 3: Goto position (most critical — oracle mandatory) ─────────
    ...
    if not resp.ok or td.decision in ("ABORT", "REPLAN"):
        # Re-read position and try once more
