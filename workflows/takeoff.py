...
Each step is checked by the planner's reflex tier before advancing.
CONNECTED → ARMED → TAKEOFF are the highest-risk state transitions, so every
hard safety gate is evaluated between steps — deterministically, in
microseconds.  The oracle is consulted only when the reflex tier cannot
classify an outcome (see planner.Planner.deliberate_step).
"""
...
async def run_takeoff(dispatcher, sm, safety, planner, altitude: float = 10.0) -> bool:
    logger.info(f"[TAKEOFF] Initiating oracle-guided takeoff to {altitude}m")
    ...
    for tool_name, args in steps:
        ...
        if not ok:
            ...
            return False
        logger.info(f"[TAKEOFF] ✓ {tool_name} — data={data}")

    logger.info(f"[TAKEOFF] ✓ Complete — hovering at {altitude}m")
    logger.info(f"[TAKEOFF] Decision tiers: {planner.decision_stats()}")
    return True
