...
# A single 10 s budget used to guarantee a timeout for every blocking wait_*
# tool: the MAVLink adapter waits up to 60 s / 120 s for altitude / arrival, so
# the dispatcher cut it off at 10 s, returned next_action="retry", and burned a
# retry (plus an LLM round trip) on every takeoff — while the orphaned thread
# kept running.  The budget now comes from the backend, which is the only thing
# that knows how long its own waits take.
...
class ToolDispatcher:
    ...
    async def dispatch(self, tool_name: str, args: dict) -> ToolResponse:
        ...
        deadline = self._adapter.timeout_for(tool_name, args)
        try:
